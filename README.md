...
- Console interface (`tictactoe.TicTacToe3DDemo`) that renders layers, offers cards immediately after each placement, and lets players place, capture, or end their turn.
...
- Requires each player to place exactly one piece; immediately after a successful placement the player draws two distinct random cards and must play one right away.
- Accepts actions while taking a turn:
  - `place`: supply `x y z` coordinates (0-indexed) to drop a piece (mandatory once per round).
  - `capture`: move an empowered piece onto an adjacent enemy cell (optional after placing).
